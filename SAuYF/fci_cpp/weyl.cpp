...
int
minimal_val(const int pos,const std::vector<int> frame, std::vector<int> tableau)
{
...
advance_weyl_tab(std::vector<int>& tableau, const std::vector<int> frame, const int M)
{
	...
	for(int k=0; k < 1000; k++)
	{
		if( tableau[pos] < M)
		{
			...
			{
				std::cout <<
				"returning advanced pos " << pos << " to  T[ans]=" << tableau[pos] << "\n";
...
*/
	...
	while(1)
	{
		...
		if(tableau[pos] >= M)
		{
			...
			{
				int min_val=minimal_val(p,frame,tableau);
				if(min_val==M)
				{
					//not standard
			...
			//reset
			tableau[pos]=minimal_val(pos,frame,tableau);
